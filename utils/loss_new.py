...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        # Define criteria
        self.focal_cls = FocalLoss(alpha=0.25, gamma=2.0)
        self.focal_obj = FocalLoss(alpha=0.5, gamma=2.0)
    ...
    def __call__(self, p, targets):
        ...
        for i, pi in enumerate(p):  # Layer index, layer predictions
            ...
            if n:
                ...
                lobj += self.focal_obj(pi[..., 4], tobj) * self.balance[i]  # Focal Loss for Objectness

                # Classification Loss
                if self.nc > 1:  # Only if multiple classes
                    t = torch.full_like(ps[:, 5:], self.cn, device=device)  # Targets
                    t[range(n), tcls[i]] = self.cp
                    lcls += self.focal_cls(ps[:, 5:], t)  # Focal Loss for Classification

                    # Dice Loss for classification
                    lcls += self.dice_loss(ps[:, 5:].unsqueeze(1), t.unsqueeze(1))

            obji = self.BCEobj(pi[..., 4], tobj)
            lobj += obji * self.balance[i]  # Objectness loss
            if self.autobalance:
...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # Ensure inputs and targets have 4 dimensions
        if inputs.dim() == 3:
            inputs = inputs.unsqueeze(2).unsqueeze(3)  # Add height and width dimensions if missing

        if targets.dim() == 3:
            targets = targets.unsqueeze(2).unsqueeze(3)  # Add height and width dimensions if missing

        intersection = (inputs * targets).sum(dim=(2, 3))  # Compute intersection
